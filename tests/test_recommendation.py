...
GRAPH_CONTEXT = {
    ...
    "next_skills": [{"skill_id": 2, "name": "유리수", "semester": "1학기", "description": "..."}],
}

ANALYSIS_BY_ID = {
    2: {"proficiency_level": "상", "reasoning": "priors가 꾸준히 높아 완전히 이해함."},
}
...
def test_next_skills_section_includes_student_diagnosis_when_available():
    section = _build_next_skills_section("상", GRAPH_CONTEXT, ANALYSIS_BY_ID)
    assert "학생의 숙련도: 상" in section
    assert "priors가 꾸준히 높아 완전히 이해함." in section


def test_next_skills_section_omits_diagnosis_when_not_in_analysis():
    section = _build_next_skills_section("상", GRAPH_CONTEXT, {})
    assert "학생의 숙련도" not in section
