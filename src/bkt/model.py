...
class BKTransformer(nn.Module):
    ...
    def forward(self, obs: torch.Tensor, output: torch.Tensor, lambd: list[float] | None = None):
        if lambd is None:
            lambd = [
        ...
        corrects = torch.zeros(batch_size, seq_len, self.n_skills, device=obs.device, dtype=obs.dtype)
        ...
        if mask.any():
            ...
            prediction_loss = torch.zeros((), device=obs.device, dtype=obs.dtype)
    ...
    def _regularization_loss(self, params: torch.Tensor, lambd: list[float]) -> torch.Tensor:
        ...
        consistency = F.relu(learn - (1 - slip) / (guess + 1e-6)).mean()
