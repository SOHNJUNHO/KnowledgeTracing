...
class BKTransformer(nn.Module):
    ...
    def forward(self, obs, output, lambd=[50, 50, 50, 1]):
        ...
        # logits: (n_skills, 5) — dim 0 is initial knowledge, dims 1-4 are BKT params
        logits = self.skill_params(self.skill_emb(skill_indices))

        oparams = logits[..., 1:].reshape(1, -1)

        # params: (B, T, n_skills, 4) — personalized BKT params per student per timestep
        # param indices: 0=learn, 1=forgetting(unused), 2=guess, 3=slip
        params = torch.sigmoid(oparams + logit_diff).view(B, T, -1, 4)

        # oparams: (n_skills, 4) — global static BKT params per skill
    ...
    def extract_latent_correct(self, params, latent, true_correct, skills):
        # param indices: 0=learn, 1=forgetting(unused/0), 2=guess, 3=slip
